      ...
      <div class="row">
        ...
        <div class="form-group col-md-4">
          ...
          <input type="text" class="form-control" 
                 value="{{ $billOfMaterial->createdBy->nama ?? $billOfMaterial->createdBy->name }} ({{ $billOfMaterial->createdBy->nip ?? $billOfMaterial->createdBy->email }})" readonly>
          <small class="form-text text-muted">Pembuat BOM</small>
        </div>
        <div class="form-group col-md-4">
          <label>Status Saat Ini</label>
          <input type="text" class="form-control" value="{{ $billOfMaterial->status }}" readonly>
      ...
      <div class="table-responsive">
        <table class="table table-bordered" id="itemTable">
          ...
          <tbody>
            ...
            <tr data-row="{{ $index }}">
              <td>
                <select class="form-control material-select select2" name="items[{{ $index }}][material_id]" required style="width: 100%;">
...
$(document).ready(function() {
  ...
  // FIXED: Force auto-fill untuk existing items setelah Select2 ready
  ...
  $(document).on('change', '.material-select', function() {
    ...
    if ($option.val()) {
      ...
      $qtyInput.val('').removeClass('bg-light').prop('readonly', false);
    }
  });
  ...
  // Add new item (sama seperti create)
  function addNewItem() {
    const template = `
